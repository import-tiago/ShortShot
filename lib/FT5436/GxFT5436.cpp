...
GxFT5436::GxFT5436(int8_t rst) : Wire(1), _sda(PB7), _scl(PB6), _rst(rst) {
	_prev_idx = 0;
	_act_idx = 1;
	_info[0].clear();
	_info[1].clear();
...
GxFT5436::GxFT5436(int8_t sda, int8_t scl, int8_t rst) : Wire(sda == PB11 ? 2 : 1), _sda(sda), _scl(scl), _rst(rst) {
	_prev_idx = 0;
	_act_idx = 1;
	_info[0].clear();
	_info[1].clear();
...
GxFT5436::GxFT5436(int8_t rst) : Wire(0), _sda(SDA), _scl(SCL), _rst(rst) {
	_prev_idx = 0;
	_act_idx = 1;
	_info[0].clear();
	_info[1].clear();
...
GxFT5436::GxFT5436(int8_t sda, int8_t scl, int8_t rst) : Wire(0), _sda(sda), _scl(scl), _rst(rst) {
	_prev_idx = 0;
	_act_idx = 1;
	_info[0].clear();
	_info[1].clear();
...
void GxFT5436::_rotate_coordinate(uint16_t& x, uint16_t& y) {
#define WIDTH 320
#define HEIGHT 480
...
#define TS_RT 0
	...
	switch (ROTATE_TOUCH_ORIENTATION) {
		case 0:
			new_x = map(x, TS_LEFT, TS_RT, 0, WIDTH);
			new_y = map(y, TS_TOP, TS_BOT, 0, HEIGHT);
			break;
		case 1:
			new_x = map(y, TS_TOP, TS_BOT, 0, WIDTH);
			new_y = map(x, TS_RT, TS_LEFT, 0, HEIGHT);
			break;
		case 2:
			new_y = map(x, TS_RT, TS_LEFT, 0, WIDTH);
			new_x = map(y, TS_BOT, TS_TOP, 0, HEIGHT);
			break;
		case 3:
			new_x = map(y, TS_BOT, TS_TOP, 0, WIDTH);
			new_y = map(x, TS_LEFT, TS_RT, 0, HEIGHT);
			break;
	}
...
void GxFT5436::init(Stream* pDiagnosticOutput) {
	...
	/* 	if (_rst >= 0) {
		...
		//Wire.begin(uint8_t(_sda), uint8_t(_scl)); // doesn't work, reason unknown, should match void begin(uint8_t, uint8_t);
	Wire.begin((uint8_t)_sda, (uint8_t)_scl); // this works
...
#endif
	//Wire.begin();
	Wire.beginTransmission(FT5436_I2C_ADDR);
	uint8_t error = Wire.endTransmission();
	if (error != 0) {
		DiagOut.print("GxFT5436::init() - Wire failed for address 0x"); DiagOut.print(FT5436_I2C_ADDR, HEX);
	}
